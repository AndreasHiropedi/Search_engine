...
class HitStreamQ:
    ...
    def __init__(self,itemStreams,lineWindow, heap = []):
        self.itemStreams = itemStreams
        self.lineWindow = lineWindow
        # create the heap to store all the itemStreams head items
        self.heap = heap
        for i in range(len(itemStreams)):
            # if the list is empty, don't bother adding it to the heap
            if itemStreams[i] != []:
                heapq.heappush(self.heap, (itemStreams[i].pop(), i))
        # begin by locating the earliest item in any stream:
        self.prev = self.nextItem()
        # store latest hit returned, so we can suppress duplicates:
        self.latestHit = None
    ...
    def nextItem(self):
        ...
        # get the index of that itemStream
        earliest_index = earliest[1]
        # find the first not None recurrence of that word in its itemStream
        if (self.itemStreams[earliest_index].peek()) is not None:
            # and push it onto the heap
            heapq.heappush(self.heap, (self.itemStreams[earliest_index].pop(), earliest_index))
        # returnt the smallest element in the heap (earliest recurrence of any word)
        return earliest
